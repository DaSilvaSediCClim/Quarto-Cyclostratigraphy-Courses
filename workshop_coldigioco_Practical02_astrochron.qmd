---
title: "Time-Series Analysis of Paleoclimate Signals – Practical 02 - Basics on astrochron"
...
Complete information can be found there:
[https://cran.r-project.org/web/packages/astrochron/astrochron.pdf](https://cran.r-project.org/web/packages/astrochron/astrochron.pdf)

![Map of astrochron function](MapAstrochron.png){#fig-map-astro}

**Citation:**
Meyers, S.R. (2014). Astrochron: An R Package for Astrochronology.[https://cran.r-project.org/package=astrochron](https://cran.r-project.org/package=astrochron)
...
dataAND89<- iso(dataAND, xmin=800, xmax=900, verbose=F, genplot=F)
...
per_sol <- periodogram(sol_insol) 

# MTM allows to include tapers and is combined with confidence level [@Thomson1982b]
mtm_sol <- mtm(sol_insol, output=1, genplot=T, verbose=F) 

# you can change the output of the mtm function and then check what is inside with 
# View(mtm_sol)
...
::: {.callout-note title="? Question ?"}
Do you see any link between the cycles and bundles you have identified visually and those that come out of the periodogram and MTM ?
:::
...
plot(mtm_AND89_i$Frequency, mtm_AND89_i$Power, type='l', log = "y", 
     xlim=c(0,10), ylim=c(0.0000001, 0.05))
...
If you want to zoom on the low frequency band, you can change the xlim argument in the plot function. 
:::
...
dataAND89_trim <- trim(dataAND89, genplot=T, verbose=F)

# Remove outliers from stratigraphic series, using specified threshold value
dataAND89_trimAT<-trimAT(dataAND89, thresh=8, dir=2, genplot=T, verbose=F)
...
dataAND89_d <- detrend(dataAND89, genplot=T, output=1, verbose=F) 
dataAND89_d2 <- detrend(dataAND89, genplot=F, output=2, verbose=F) 

```

::: {.callout-note title="? Question ?"}
You can see that in the function detrend, the argument output can be set to 1 or 2, what does it change ?
:::
...
dataAND89_nL2 <- noLow(dataAND89, smooth=0.2, genplot=T, output=2, verbose=F)
...
::: {.callout-note title="? Question ?"}
In the function noLow, can you see how the parameter "smooth" changes how the smoothing is applied ?
:::
...
lines(mtm_AND89_logi$Frequency, mtm_AND89_logi$Power*1000, col='darkred')
lines(mtm_AND89_logi$Frequency, mtm_AND89_logi$AR1_95_power*1000, lty=2, col='darkred')
...
```
We see multiple peaks in the mtm,round 0.05 1/m (corresponding to 20 m) and around 0.25 1/m (corresponding to ~4 m). 
These two cycles have a ratios of 1/5 and have been interpreted respectively as short eccentricity and precession cycles. So the next step is to filter those cycles and to look at their amplitude modulation.  
...
Now we provide another dataset of CaCO<sub>3</sub>. 
...
::: {.callout-note title="? Question ?"}
Can you do the same exercise. Can you identify the phase relationship ? is it the same or different ? how do you link the phrase relationship between orbital forcing with clasts and with CaCO<sub>3</sub> ?
::: 