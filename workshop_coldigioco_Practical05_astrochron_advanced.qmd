---
title: "Time-Series Analysis of Paleoclimate Signals – Practical 05 - Advanced techniques on astrochron"
...
Note: These ‘recipe’ should be taken with caution; they are given with very basic settings, and settings fitting with the studied data. We will also see that it can be challenging. Always go back to the original papers and learn to use those tools properly and not as black boxes. 
...
DataA_d <- noLow(DataA_i002, smooth=0.2)
...
eha_A <- eha(DataA_d, win = 7, pad = 1000, step = 0.018, 
             output = 4, fmax = 3, genplot=2, verbose = F)

# Now let's test the impact of trimming on the EHA 
DataAT <- trim(DataisoA)

# We interpolate our trimmed signal 
DataAT_i <- linterp(DataAT, verbose=F)

# Re-run EHA on trimmed data
eha_A <- eha(DataAT_i, win = 7, pad = 1000, step = 0.018, 
             output = 4, fmax = 3, genplot=2, verbose = F)

```

::: {.callout-note title="? Question ?"}
Check the role of every parameter in the function, mostly "win", "output" and "fmax". What are the implications of those parameters ? <br /> 
What is the impact of the noLow (polynomial detrending) function on the results of the eha for this specific record and why ? 
:::


## ASM - Average Spectral Misfits or on the danger of applying a technique blindly 
The Average Spectral Misfits [@Meyers2007a] provides an objective estimate of the optimal sedimentation rate for a stratigraphic interval that preserves a record of orbital forcing. The technique also provides a formal statistical test for rejecting the null hypothesis (no orbital signal). 
...
# To identify the peaks you think are significant, you can select those manually in R (not in Quarto, so you have to uncomment the following lines). Be careful to select them from the lowest frequency to the highest frequency) :
...
## TimeOpt 
The TimeOpt technique [@Meyers2015c] identifies the time scale that simultaneously optimizes eccentricity amplitude modulation of the precession band, and the concentration of power at precession (carrier) and eccentricity (modulator) frequencies. 
```{r}
#| echo: true
#| results: hide
#| warning: false
...
::: {.callout-note title="? Question ?"}
The outcome is again different from ASM and is different depending how you define the P frequencies. How would you test which ASM outcome (sedimentation rate) is best ?
:::
...
If you want to practice further, you can consider those cases from the Cyclostratigraphic Intercomparison Project, made available on [https://www.cyclostratigraphy.org/the-cip-initiative](https://www.cyclostratigraphy.org/the-cip-initiative). Don't look at the answers and get your hand durty. 
